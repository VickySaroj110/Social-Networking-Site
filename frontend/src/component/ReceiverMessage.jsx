...
function ReceiverMessage({message}) {
  ...
  const handleShare = async () => {
    try {
      const postId = message.postId?._id;
      const loopId = message.loopId?._id;
      ...
      // For now, copy the share link to clipboard or show a message
      const shareText = `Check out this ${postId ? 'post' : 'reel'}!`;
      
      // Copy to clipboard (you can also open a modal to select users)
      await navigator.clipboard.writeText(shareText);
      alert("Post shared! (Copied to clipboard - you can share with other users)");
  ...
  const handleDeleteMessage = async () => {
    try {
      ...
      // Update Redux state by filtering out the deleted message
  ...
  return (
    <div className='w-fit max-w-[500px] left-0 flex flex-col gap-[10px]'>
      ...
      {isLoop && post && (
        <div className='w-[400px] h-[600px] flex flex-col gap-0 bg-black rounded-2xl overflow-visible relative'>
